...
async fn test_load_json() {
    ...
    let enabled: bool = config.get("database.enabled").unwrap();
    assert!(enabled);
}
...
async fn test_nested_access() {
    ...
    let host: String = config.get("database.connection.host").unwrap();
    assert_eq!(host, "db.example.com");

    let port: u16 = config.get("database.connection.port").unwrap();
    assert_eq!(port, 5432);
}
...
async fn test_key_not_found() {
    ...
    let result: Result<String, Error> = config.get("nonexistent.key");
...
async fn test_type_conversion() {
    ...
    let port_u16: u16 = config.get("server.port").unwrap();
    assert_eq!(port_u16, 8080);

    let port_u32: u32 = config.get("server.port").unwrap();
    assert_eq!(port_u32, 8080);

    let port_i32: i32 = config.get("server.port").unwrap();
...
async fn test_conversion_error() {
    ...
    let result: Result<bool, Error> = config.get("app.name");
    assert!(result.is_err());
}
